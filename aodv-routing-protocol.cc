...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("IsMalicious", "Is the node malicious",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("IsGrayhole", "Indicates whether the node exhibits grayhole behaviour",
                   BooleanValue (false),
                   MakeBooleanAccessor (&RoutingProtocol::SetGrayhole,
                                        &RoutingProtocol::GetGrayhole),
                   MakeBooleanChecker ())
  ;
  return tid;
}
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  /* Code added by Marilyn Mboa */
  std::srand(static_cast<unsigned int>(std::time(0)));  // Seed the random number generator

  if(IsMalicious)
  {        
    // Assign either true or false randomly
    IsGrayhole = rand() % 2 == 0;  
    
    if(IsGrayhole){
        
        //get the routing table of the current node
        std::map<Ipv4Address, RoutingTableEntry> tableEntries = m_routingTable.getIpv4AddressEntry();
        
        //Loop through the entries to check if current node has a malicious neighour
        for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = tableEntries.begin();
            i != tableEntries.end(); ++i)
        {
            Ipv4Address nextHop = i->second.GetNextHop(); 
            std::ostringstream oss;
            oss << nextHop;
            std::string nextHopStr = oss.str();
            
            if (IsMyOwnAddress(nextHop) || nextHopStr.rfind("127.", 0) == 0)
            {
                continue; // The next hop is my own address
            }
            
            int32_t nextHopInterfaceIndex = m_ipv4->GetInterfaceForAddress(nextHop);
            if (nextHopInterfaceIndex == -1)// No valid interface for the next hop address
            { continue; }
            
            Ptr<NetDevice> nextHopNetDevice = m_ipv4->GetNetDevice(nextHopInterfaceIndex);
            if (nextHopNetDevice == NULL) // No valid net device for the interface
            { continue; }
            
            Ptr<Node> nextHopNode = nextHopNetDevice->GetNode();
            if (nextHopNode == NULL) // No valid node associated with the net device
            { continue; }
            
            Ptr<RoutingProtocol> nextHopNodeProtocol = nextHopNode->GetObject<RoutingProtocol>();
            
            if (nextHopNodeProtocol->IsMalicious)
            {  //Forwarding to malicious neighbour
                Ipv4Address malicious_dst = nextHopNode->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal();
                if (m_routingTable.LookupRoute (malicious_dst, toDst)){
                    std :: cout <<"Yes the lookup for mdst and toDst is valid \n";
                    if (toDst.GetFlag () == VALID)
                    {
                        Ptr<Ipv4Route> route = toDst.GetRoute ();
                        
                        //Update route lifetimes to maintain the appearance of normal behavior
                        UpdateRouteLifeTime (origin, ActiveRouteTimeout);
                        UpdateRouteLifeTime (malicious_dst, ActiveRouteTimeout);
                        UpdateRouteLifeTime (route->GetGateway (), ActiveRouteTimeout);
        ...
        std::cout <<"No malicious neighbour! Packet dropped . . . \n";
         return false ;
    }//End If Isgrayhole
    
    std::cout <<"No malicious neighbour! Packet dropped . . . \n";
    return false;
  } // End if IsNodeMalicious
